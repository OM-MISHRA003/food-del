...
async function seedIfEmpty(db) {
    ...
    if (count === 0) {
        console.log("Database is empty, seeding food data...");
        await db.collection("foods").insertMany(foodData);
        console.log("Seeded", foodData.length, "food items!");
    }
}
...
export const connectDB = async () => {
    try {
        ...
        // Auto-seed food data if the database is empty
        await seedIfEmpty(mongoose.connection.db);
    } catch (err) {
